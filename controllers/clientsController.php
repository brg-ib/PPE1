...
switch($action)
{
	...
	case 'form_password_forgot':  form_password_forgot();  break;
	case 'forgot': $mailUser=$_REQUEST['mailUser']; forgot($bdd,$mailUser);  break;
	case 'form_inscription': form_inscription(); break;
	case 'get':$idClient=$_GET['idClient']; get_client($bdd,$idClient); break;
	case 'add': $nomUser=$_REQUEST['nomUser']; $prenomUser=$_REQUEST['prenomUser']; $telUser=$_REQUEST['telUser']; $passwordUser=$_REQUEST['passwordUser']; $mailUser=$_REQUEST['mailUser']; ajouter_client($nomUser,$prenomUser,$mailUser,$passwordUser,$telUser,$bdd); header('Location:index.php');  break;
	...
	case 'update':$idUser=$_GET['idUser']; $nomUser=$_REQUEST['nomUser']; $prenomUser=$_REQUEST['prenomUser']; $telUser=$_REQUEST['telUser']; update_client($nomUser,$prenomUser,$telUser,$idUser,$bdd);  header('Location:index.php?module=clients');  break;
	case 'delete': $idClient=$_REQUEST['idClient']; delete_client($bdd,$idClient); header('Location:index.php?module=clients');  break;
